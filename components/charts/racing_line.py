...
def _resample_in_time(slc, n_frames):
    ...
    if total_t < 1e-6:
        ...
        return (_resample_spatial(x, n_frames),
...
def build(active_tels: dict, mode: str = "team", n_frames: int = 100) -> go.Figure:
    ...
    for drv, info in valid.items():
        ...
        li    = info["lap_info"]
        label = (f"{drv}  L{li.get('lap_number','?')}  "
                 f"{li.get('compound','')} ({li.get('tyre_life',0)}L)")

        if mode == "speed":
            fig.add_trace(go.Scatter(
                x=slc["X"], y=slc["Y"], mode="markers",
                marker=dict(
                    size=5,
                    color=slc["Speed"],
                    colorscale=[
            ...
            fig.add_trace(go.Scatter(
                x=slc["X"], y=slc["Y"], mode="lines",
                line=dict(color=color, width=6), opacity=0.12,
    ...
    fig.add_trace(go.Scatter(
        x=[apex["X"]], y=[apex["Y"]], mode="markers",
        marker=dict(size=10, color="#fff", symbol="x",
                    line=dict(width=2, color="#fff")),
        name="Apex", showlegend=False, hoverinfo="skip",
    ))
    ...
    fig.update_layout(
        ...
        legend=dict(
            bgcolor="rgba(8,9,13,0.95)", bordercolor="#1a1d24", borderwidth=1,
...
def _add_track_layers(fig, ref_slc):
    ...
    for width, color in [(36, "#14171e"), (26, "#1e2229"), (16, "#262c36")]:
        fig.add_trace(go.Scatter(
            x=ref_slc["X"], y=ref_slc["Y"], mode="lines",
