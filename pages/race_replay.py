...
from components.shared import (
    BG2, GRID, TEXT, ACCENT, FONT,
    get_cached_session, get_driver_meta,
)
...
def _load_replay_data(session, drivers):
    ...
    for drv in drivers:
        try:
            ...
            valid_laps = (laps_drv[laps_drv["LapTime"].notna()]
...
def build_static_fig(ref_x, ref_y, driver_data, dlist):
    ...
    for w, c in [(32, "#14171e"), (22, "#1e2229"), (13, "#262c36")]:
        fig.add_trace(go.Scatter(x=tx, y=ty, mode="lines",
            line=dict(color=c, width=w), hoverinfo="skip", showlegend=False))
    for w, op in [(24, 0.11), (16, 0.06)]:
        fig.add_trace(go.Scatter(x=tx, y=ty, mode="lines",
            line=dict(color="#fff", width=w), opacity=op,
            hoverinfo="skip", showlegend=False))

    # S/F line
    fig.add_trace(go.Scatter(
        x=[tx[0]], y=[ty[0]], mode="markers+text",
        marker=dict(size=12, color="#fff", symbol="line-ns",
    ...
    fig.update_layout(
        ...
        legend=dict(bgcolor="rgba(8,9,13,0.95)", bordercolor="#1a1d24",
                    borderwidth=1, font=dict(size=11, color="#ccc"),
                    orientation="h", x=0, y=1.06),
        hoverlabel=dict(bgcolor="#1a1d24", bordercolor=GRID,
...
def add_animation(fig, driver_data, dlist, dot_indices, speed=1.0):
    ...
    fig.frames = [
        go.Frame(
            data=[
                trace
                for drv in dlist
                for trace in _frame_traces(driver_data[drv], fi)
            ],
            name=str(fi),
            traces=dot_indices,
        )
    ...
    for lap in range(total_laps + 1):
        ...
        lap_steps.append({
            "args": [[str(fi)], {"frame": {"duration": 0, "redraw": False},
    ...
    fig.update_layout(
        updatemenus=[{
            ...
            "y": -0.13, "x": 0.5, "xanchor": "center",
            "bgcolor": "#1a1d24", "bordercolor": GRID,
        ...
        sliders=[{
            "active": 0, "steps": lap_steps,
            "y": -0.07, "len": 0.86, "x": 0.07,
            "currentvalue": {"prefix": "Lap: ", "font": {"color": "#888", "size": 10},
...
def _frame_traces(d, fi):
    ...
    return [
        ...
        go.Scatter(x=[x], y=[y], mode="markers+text",
            ...
            text=[d.get("drv", "")], textposition="top center",
            textfont=dict(size=9, color="#fff")),
    ]
        ...
        html.Div(
            ...
            children=[
                    ...
                    children=[
                        html.Div(style={"display":"flex","alignItems":"center","gap":"8px"},
                            ...
                            html.Div(style={"width":"3px","height":"18px","background":ACCENT,
                                             "borderRadius":"2px"}),
                            html.Div("2D RACE REPLAY", style={"fontSize":"11px","fontWeight":"700",
                        ...
                        html.Div(
                            ...
                            children=[
                                ...
                                dcc.RadioItems(
                                    ...
                                    labelStyle={"marginLeft": "10px", "fontSize": "11px",
                ...
                html.Div(
                    ...
                    style={"fontSize": "11px", "color": "#444", "marginBottom": "10px"},
                ),
                dcc.Loading(type="circle", color="#e8002d",
                         children=html.Div(id="replay-chart-wrap",
                         children=[dcc.Graph(
                             figure=build_replay(session_key, drivers, speed=1.0),
...
def update_speed(speed, session_key, selected_drivers):
    if not session_key:
        import dash; raise dash.exceptions.PreventUpdate
