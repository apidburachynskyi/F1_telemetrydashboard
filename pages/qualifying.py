...
from components.shared import (
    BG2, BG3, GRID, TEXT, MUTED, ACCENT, FONT,
    team_logo_img,
)
...
def _th(text):
    return html.Th(text, style={"fontSize":"9px","fontWeight":"700","letterSpacing":"1.8px",
                                  "color":"#444","padding":"8px 12px","textAlign":"left",
                                  "borderBottom":f"1px solid {GRID}",
                                  "textTransform":"uppercase","whiteSpace":"nowrap"})

def _td(content, bold=False, color=None, size="12px"):
    return html.Td(content, style={"padding":"7px 10px","fontSize":size,
                                    "color":color or TEXT,"borderBottom":f"1px solid {BG3}",
                                    "fontWeight":"700" if bold else "400"})

def _pos_badge(pos, elim=False):
    colors = {1:"#ffd700",2:"#c0c0c0",3:"#cd7f32"}
    if elim: bg,tc = "#1a0508",ACCENT
    elif pos in colors: bg,tc = colors[pos],"#000"
    else: bg,tc = BG3,TEXT
    border = f"1px solid {ACCENT}" if elim else ("none" if pos in colors else f"1px solid {GRID}")
    return html.Div(str(pos), style={"width":"28px","height":"28px","borderRadius":"5px",
                                      "background":bg,"color":tc,"fontWeight":"700",
                                      "fontSize":"12px","display":"flex","border":border,
                                      "alignItems":"center","justifyContent":"center"})

def _driver_cell(r):
    color = r["color"]
    return html.Div(style={"display":"flex","alignItems":"center","gap":"7px"}, children=[
        html.Div(style={"width":"3px","height":"26px","background":color,"borderRadius":"2px"}),
        html.Div([
            html.Div(r["first"].upper(), style={"fontSize":"9px","color":"#555"}),
            html.Div(r["last"].upper(),  style={"fontSize":"12px","fontWeight":"700","color":color}),
        ]),
    ])
...
def _segment_table(rows, advance_count=None):
    ...
    for r in rows:
        ...
        table_rows.append(html.Tr(
            style={"opacity":"0.4" if elim else "1",
                   "transition":"background 0.12s",
                   "borderBottom":f"1px solid {BG3}"},
            children=[
                html.Td(_pos_badge(pos,elim), style={"padding":"7px 8px","width":"32px"}),
                html.Td(_driver_cell(r), style={"padding":"7px 8px"}),
                html.Td(
                    html.Div(style={"display":"flex","alignItems":"center","gap":"4px"},
                             children=[logo for logo in [team_logo_img(r["team"],"14px")] if logo]
                                       + [html.Span(r["team"])]),
                    style={"padding":"7px 10px","fontSize":"10px","color":"#888",
                            "borderBottom":f"1px solid {BG3}","maxWidth":"110px",
                            "overflow":"hidden","textOverflow":"ellipsis","whiteSpace":"nowrap"}),
                html.Td(r["seg_time"], style={"padding":"7px 10px","fontSize":"12px",
                                               "fontWeight":"600","color":TEXT,
                                               "borderBottom":f"1px solid {BG3}"}),
                html.Td(r["seg_gap"], style={"padding":"7px 10px","fontSize":"11px",
                                              "color":"#888","borderBottom":f"1px solid {BG3}"}),
            ]
        ))
    return html.Table(style={"width":"100%","borderCollapse":"collapse"}, children=[
        html.Thead(html.Tr([_th(h) for h in ["P","DRIVER","TEAM","TIME","GAP"]])),
...
def _seg_header(title, color=ACCENT):
    return html.Div(
        style={"display":"flex","alignItems":"center","gap":"7px",
               ...
               "borderBottom":f"1px solid {GRID}"},
        children=[
            html.Div(style={"width":"3px","height":"18px","background":color,
...
def build_timeline(results, segment="Q1"):
    ...
    fig.update_layout(
        ...
        height=440, margin=dict(l=84, r=28, t=20, b=88),
        hovermode="closest",
        hoverlabel=dict(bgcolor="#13161e", bordercolor="#252830",
                        font=dict(color=TEXT, size=11)),
        showlegend=False,
        xaxis=dict(gridcolor="#161920", zeroline=False,
                   tickfont=dict(size=10, color="#777"), tickangle=-42,
                   showline=True, linecolor="#1a1d24",
                   title=dict(text="Driver", font=dict(size=10, color="#666"))),
        yaxis=dict(gridcolor="#161920", zeroline=False,
...
def _wcard(icon, label, value):
    return html.Div(style={"background":BG2,"border":f"1px solid {GRID}","borderRadius":"6px",
                            "padding":"10px 14px","flex":"1"}, children=[
        html.Div(style={"display":"flex","alignItems":"center","gap":"5px","marginBottom":"3px"},
                 children=[html.Span(icon,style={"fontSize":"12px"}),
                            html.Span(label,style={"fontSize":"9px","fontWeight":"700",
                                                    "letterSpacing":"1.5px","color":"#444"})]),
        html.Div(value, style={"fontSize":"15px","fontWeight":"700","color":TEXT}),
    ])
...
def empty():
    return html.Div(style={"display":"flex","flexDirection":"column","alignItems":"center",
        ...
        html.Div("LOAD A QUALIFYING SESSION",
                 style={"fontSize":"14px","fontWeight":"700","letterSpacing":"2px","color":TEXT}),
        html.Div("Select Qualifying from the session type dropdown.",
...
def render(store):
    if not store: return empty()
    if store.get("session_type","Race") == "Race":
        return html.Div(style={"display":"flex","flexDirection":"column","alignItems":"center",
                                "justifyContent":"center","minHeight":"50vh","gap":"12px"}, children=[
            html.Div("🏁", style={"fontSize":"36px"}),
            html.Div("Race session loaded.",
                     style={"fontSize":"14px","fontWeight":"700","color":TEXT}),
            html.Div("Switch to the Overview tab to see race results.",
                     style={"fontSize":"12px","color":"#555"}),
        ])

    results = store.get("results", [])
    if not results:
        return html.Div("No qualifying data available.",
    ...
    def wfmt(key,unit):
        v=w.get(key); return f"{v}{unit}" if v else "–"
    ...
    results_section = html.Div(
        style={"background":BG2,"border":f"1px solid {GRID}","borderRadius":"6px",
               "padding":"16px","marginBottom":"14px"},
        children=[
            html.Div("QUALIFYING RESULTS",
                     style={"fontSize":"12px","fontWeight":"700","letterSpacing":"2px",
                             "color":TEXT,"borderLeft":f"3px solid {ACCENT}",
                             "paddingLeft":"10px","marginBottom":"16px"}),
            html.Div(style={"display":"grid","gridTemplateColumns":"1fr 1fr 1fr","gap":"16px"}, children=[
                html.Div([_seg_header("Q3"), _segment_table(q3, advance_count=None)]),
    ...
    seg_selector = html.Div(
        ...
        children=[
            html.Button(seg, id=f"quali-seg-{seg}", n_clicks=0, style={
    ...
    timeline_section = html.Div(
        style={"background":BG2,"border":f"1px solid {GRID}","borderRadius":"6px","padding":"16px"},
        children=[
            html.Div("QUALIFYING TIMELINE",
                     style={"fontSize":"12px","fontWeight":"700","letterSpacing":"2px",
                             "color":TEXT,"borderLeft":f"3px solid {ACCENT}",
                             "paddingLeft":"10px","marginBottom":"12px"}),
            seg_selector,
            html.Div(id="quali-timeline-chart",
                     children=[dcc.Graph(figure=build_timeline(results,"Q1"),
                                         config={"displayModeBar":True})]),
            html.Div("● advanced  ○ eliminated  — red line = cutoff",
...
def update_timeline(q1c, q2c, q3c, store):
    ...
    triggered = ctx.triggered[0]["prop_id"] if ctx.triggered else "quali-seg-Q1.n_clicks"
    if   "Q3" in triggered: seg = "Q3"
    elif "Q2" in triggered: seg = "Q2"
    else:                   seg = "Q1"

    results = store.get("results",[])
    return dcc.Graph(figure=build_timeline(results, seg),
                     config={"displayModeBar":True})
