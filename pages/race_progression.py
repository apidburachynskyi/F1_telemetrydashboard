...
from components.shared import (
    ...
    chart_theme, axis_label, format_laptime,
)
...
def _chart_card(title, fig):
    return html.Div(
        style={"background": BG2, "border": f"1px solid {GRID}",
               "borderRadius": "8px", "padding": "18px 20px", "marginBottom": "14px",
               "boxShadow":"0 2px 12px rgba(0,0,0,0.3)"},
        children=[_section(title),
...
def lap_time_comparison(laps, drivers, drv_colors):
    ...
    fig.update_layout(
        ...
        height=440, margin=dict(l=60, r=80, t=24, b=52),
        hoverlabel=dict(bgcolor="#13161e", bordercolor="#252830", font=dict(color="#e0e0e0", size=11)),
        legend=dict(bgcolor="rgba(13,15,20,0.9)", bordercolor="#1a1d24", borderwidth=1,
                    font=dict(size=11, color="#ccc"), orientation="h", x=0, y=1.06),
        xaxis=dict(gridcolor="#161920", zeroline=False, tickfont=dict(size=10, color="#777"),
                   showline=True, linecolor="#1a1d24"),
        yaxis=dict(gridcolor="#161920", zeroline=False, tickfont=dict(size=10, color="#777"),
                   showline=True, linecolor="#1a1d24"),
        hovermode="x unified",
...
def driver_stat_cards(laps, drivers, driver_meta):
    ...
    for drv in drivers:
        ...
        drv_laps = [l["LapTimeSec"] for l in laps
        ...
        fastest_lap = next(
            (l["LapNumber"] for l in laps
             if l["Driver"] == drv and l.get("LapTimeSec") == fastest),
            "?",
        )
        cards.append(html.Div(
            style={"flex": "1", "background": BG2, "border": f"1px solid {GRID}",
            ...
            children=[
                html.Div(
                    ...
                    children=[
                        ...
                        html.Span(drv, style={"fontSize": "14px", "fontWeight": "700",
                                               "color": TEXT}),
                    ],
                ),
                html.Div([
                    html.Span("FASTEST ", style={"fontSize": "9px", "color": "#444",
                                                  "letterSpacing": "1px"}),
                    html.Span(format_laptime(fastest),
                              style={"fontSize": "13px", "fontWeight": "700", "color": TEXT}),
                    html.Span(f" (L{fastest_lap})",
                              style={"fontSize": "10px", "color": "#555"}),
                ], style={"marginBottom": "4px"}),
                html.Div([
                    html.Span("AVG ", style={"fontSize": "9px", "color": "#444",
                                              "letterSpacing": "1px"}),
                    html.Span(format_laptime(avg),
                              style={"fontSize": "13px", "fontWeight": "700", "color": "#888"}),
                ]),
            ],
        ))
    return html.Div(style={"display": "flex", "gap": "8px", "marginTop": "10px"},
                    children=cards)
...
def distribution_scatter(laps, drivers, drv_colors):
    ...
    fig = go.Figure()
    for drv in drivers:
        times = [l["LapTimeSec"] for l in laps
                 if l["Driver"] == drv and l.get("LapTimeSec")]
        if not times: continue
        color = drv_colors.get(drv, "#AAAAAA")
        fig.add_trace(go.Scatter(
            x=[drv] * len(times), y=times,
            mode="markers",
...
def consistency_iqr(laps, drivers, drv_colors):
    ...
    fig = go.Figure()
    for drv in drivers:
        times = np.array([l["LapTimeSec"] for l in laps
                          if l["Driver"] == drv and l.get("LapTimeSec")])
        if len(times) < 4: continue
        color = drv_colors.get(drv, "#AAAAAA")
        ...
        fig.add_trace(go.Scatter(
            ...
            name=drv,
            hovertemplate=(f"<b>{drv}</b>  IQR<br>"
    ...
    return html.Div([
        ...
        html.Div(
            ...
            children=[
                ...
                dcc.Graph(figure=lap_time_comparison(laps, drivers, drv_colors),
        ...
        html.Div(
            style={"display": "grid", "gridTemplateColumns": "1fr 1fr", "gap": "12px",
                   "marginBottom": "12px"},
            children=[
                _chart_card("LAP TIME DISTRIBUTION", distribution_scatter(laps, drivers, drv_colors)),
                _chart_card("CONSISTENCY (IQR)",     consistency_iqr(laps, drivers, drv_colors)),
            ],
        ),
