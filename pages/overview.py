...
def _section(text, icon=""):
    return html.Div(
        ...
        children=[
            html.Div(style={"width":"3px","height":"20px","background":ACCENT,
                             "borderRadius":"2px","flexShrink":"0"}),
            html.Div(text, style={"fontSize":"11px","fontWeight":"700",
                                   "letterSpacing":"2.5px","color":TEXT}),
        ])

def _info_card(icon, label, val):
    return html.Div(className="info-card", children=[
        html.Div([html.Span(icon,style={"marginRight":"4px","fontSize":"11px"}),
...
def _stat_card(icon, label, val, color=ACCENT):
    return html.Div(className="stat-card", children=[
        html.Span(icon, style={"fontSize":"16px","color":color,"marginTop":"2px"}),
        html.Div([
...
def empty_state():
    return html.Div(
        ...
        children=[
            html.Div("📊",style={"fontSize":"44px","background":"#12151c",
                                  "borderRadius":"14px","padding":"14px 18px",
                                  "border":f"1px solid {GRID}"}),
            html.Div("SELECT A SESSION",style={"fontSize":"14px","fontWeight":"700",
                                                "letterSpacing":"3px","color":TEXT}),
            html.Div("Choose year and GP from the sidebar, then click LOAD SESSION.",
...
def render(store, selected_drivers):
    if not store: return empty_state()
    ...
    def wfmt(k,u):
        v=weather.get(k); return f"{v}{u}" if v else "–"
    ...
    for r in results:
        ...
        if grid_val and grid_val > 0 and pos < 99:
            ...
            de = html.Span(
                ...
                className="delta-pos" if delta>0 else ("delta-neg" if delta<0 else "delta-zero"))
        elif grid_val == 0:
        ...
        comp_dots = html.Div(
            ...
            children=[
                html.Span(style={"width":"11px","height":"11px","borderRadius":"50%",
        ...
        # Gap formatting
        gap_str = r.get("gap","–")
        gap_el = html.Span(gap_str, className="gap-leader" if gap_str=="–" else "gap-time")
        ...
        rows.append(html.Tr(
            ...
            children=[
                html.Td(
                    _pos_badge(pos) if pos<99 else html.Div("–",style={"color":"#444"}),
                    style={"padding":"10px 12px","width":"44px"}),
                html.Td(
                    html.Div(style={"display":"flex","alignItems":"center","gap":"10px"},children=[
                        html.Div(style={"width":"3px","height":"34px","background":color,
                                         "borderRadius":"2px","flexShrink":"0"}),
                        html.Div([
                            html.Div(r["first"].upper(), className="drv-first"),
                            html.Div(r["last"].upper(),
                ...
                html.Td(team_cell,
                        style={"padding":"10px 12px","color":"#888","fontSize":"10px",
                               "maxWidth":"130px","overflow":"hidden",
                               "textOverflow":"ellipsis","whiteSpace":"nowrap"}),
                html.Td(comp_dots, style={"padding":"10px 12px"}),
                html.Td(gap_el,    style={"padding":"10px 12px"}),
                html.Td(
                    "PL" if r.get("grid")==0 else (f"P{r['grid']}" if r.get("grid") else "–"),
                    style={"padding":"10px 12px","color":"#555","fontSize":"10px"}),
                ...
                html.Table(className="f1-table", children=[
                    html.Thead(html.Tr([
                        html.Th(h, style={"fontSize":"9px","fontWeight":"700",
                                           ...
                                           "borderBottom":f"1px solid {GRID}",
                                           "whiteSpace":"nowrap"})
                        for h in ["POS","DRIVER","TEAM","TYRES","GAP","GRID","+/–"]
                    ])),
                    html.Tbody(rows),
                ]),
            ]),
    ...
    ret = sum(1 for r in results if any(
        kw in r.get("status","").lower()
        for kw in ["accident","retired","dnf","engine","collision","mechanical"]))
    ...
    stats_panel = html.Div(
        ...
        children=[
            ...
            html.Div(style={"display":"grid","gridTemplateColumns":"1fr 1fr","gap":"8px"},
                ...
                _stat_card("🅿","SAFETY CAR",    f"{rc.get('sc',0)} deploy.", "#f97316"),
            ]),
        ])
    ...
    return html.Div([
        weather_row,
        html.Div(
            style={"display":"grid","gridTemplateColumns":"1fr 300px","gap":"14px"},
