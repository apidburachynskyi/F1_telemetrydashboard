...
from components.shared import (
    BG2, BG3, GRID, TEXT, MUTED, ACCENT,
    get_cached_session, get_driver_meta, format_laptime,
)
...
def _sector_badge(value, is_best):
    ...
    return html.Span(
        text,
        style={"background": BG3, "color": TEXT,
...
def _build_sector_table(laps, drivers, lap_number, driver_meta):
    ...
    for drv in drivers:
        matching = [l for l in laps
                    if l["Driver"] == drv and l["LapNumber"] == lap_number]
        if not matching:
            continue
        lap = matching[0]
        ...
        for key, col in [("S1", "Sector1TimeSec"),
                          ("S2", "Sector2TimeSec"),
                          ("S3", "Sector3TimeSec")]:
            v = lap.get(col)
    ...
    best_lap = min(
        (l.get("LapTimeSec") for l in drv_lap_data.values()
         if l.get("LapTimeSec")),
        default=None,
    )
    ...
    for pos, drv in enumerate(drivers, 1):
        ...
        lap_time_el = html.Span(
            format_laptime(lt),
            style={
                "background": BG3, "padding": "4px 12px",
        ...
        rows.append(html.Tr(
            style={"borderBottom": f"1px solid {BG3}", "transition":"background 0.12s"},
            children=[
                html.Td(
                    html.Div(str(pos), style={"width":"28px","height":"28px",
                        "borderRadius":"5px","background":BG3,"color":"#888",
                        "border":f"1px solid {GRID}","fontWeight":"700","fontSize":"12px",
                ...
                html.Td(
                    html.Div(
                        ...
                        children=[
                            html.Div(style={"width": "3px", "height": "32px",
                                            "background": color, "borderRadius": "2px"}),
                            html.Div([
                                html.Div(meta.get("first", "").upper(),
                                         style={"fontSize": "10px", "color": "#555"}),
                                html.Div(meta.get("last", drv).upper(),
                ...
                html.Td(_sector_badge(lap.get("Sector1TimeSec"),
                                      lap.get("Sector1TimeSec") == sector_bests["S1"]),
                        style={"padding": "10px 12px"}),
                html.Td(_sector_badge(lap.get("Sector2TimeSec"),
                                      lap.get("Sector2TimeSec") == sector_bests["S2"]),
                        style={"padding": "10px 12px"}),
                html.Td(_sector_badge(lap.get("Sector3TimeSec"),
    ...
    return html.Table(
        ...
        children=[
            html.Thead(html.Tr([
                html.Th(h, style={"fontSize": "9px", "fontWeight": "700",
                                   "letterSpacing": "1.8px", "color": "#444",
...
layout = html.Div([
    ...
    dcc.Loading(type="circle", color="#e8002d",
        children=html.Div(id="telemetry-wrap")),
])
...
def render(store, session_key, selected_drivers):
    ...
    if not drivers:
        msg = html.Div("Select drivers from the sidebar.",
    ...
    # Lap selector bar
    selector = html.Div(
        style={"background": BG2, "border": f"1px solid {GRID}",
               "borderRadius": "8px", "padding": "14px 18px",
               "display": "flex", "alignItems": "center", "justifyContent": "space-between",
               "boxShadow": "0 2px 8px rgba(0,0,0,0.25)"},
        children=[
            html.Div(style={"display":"flex","alignItems":"center","gap":"8px"}, children=[
                html.Div(style={"width":"3px","height":"18px","background":ACCENT,
                                 "borderRadius":"2px"}),
                html.Div("LAP SELECTOR", style={"fontSize":"11px","fontWeight":"700",
                                                  "letterSpacing":"2.5px","color":TEXT}),
            ]),
            html.Div(
                style={"display": "flex", "alignItems": "center", "gap": "10px"},
                children=[
                    html.Span("SELECT LAP:", style={"fontSize": "10px", "color": "#555",
                    ...
                    html.Span(f"{len(drivers)} DRIVERS",
                              style={"fontSize": "10px", "color": "#555",
    ...
    sector_card = html.Div(
        ...
        children=[
            html.Div(style={"marginBottom":"14px","paddingBottom":"10px",
                ...
                html.Div(style={"display":"flex","alignItems":"center","gap":"8px",
                    ...
                    html.Div("SECTOR TIMES", style={"fontSize":"11px","fontWeight":"700",
                                                     "letterSpacing":"2.5px","color":TEXT}),
                ]),
                html.Div("Lap-by-lap sector performance  ·  Purple = best sector",
                         style={"fontSize":"10px","color":"#555","paddingLeft":"11px"}),
            ]),
            html.Div(id="sector-table-inner",
...
def _build_telemetry_card(session_key, drivers):
    ...
    try:
        ...
        return html.Div(f"Could not load telemetry: {e}",
                        style={"color": "#555", "padding": "20px"})
    ...
    for drv in drivers:
        try:
            ...
            if lap is None or lap.empty: continue
    ...
    if not driver_telemetry:
        return html.Div("No telemetry available.",
                        style={"color": "#555", "padding": "20px"})
    ...
    return html.Div(
        ...
        children=[
            html.Div("COMBINED TELEMETRY — FASTEST LAP PER DRIVER",
                     style={"fontSize": "12px", "fontWeight": "700",
                             "letterSpacing": "2px", "color": TEXT,
                             "borderLeft": f"3px solid {ACCENT}", "paddingLeft": "10px",
                             "marginBottom": "12px"}),
            dcc.Graph(figure=build_telemetry(driver_telemetry),
